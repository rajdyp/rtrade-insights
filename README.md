...
- Optionally enter 21-day ATR % and review risk in ATR: `stop loss % / ATR %`.
...
The CLI stores that browser session in `.browser-profile/`, which is ignored by git.
...
To fetch market data from Alpaca in one flow, set your local credentials outside the repo:
...
`SYMBOL` rows fetch price, compute a stop from today's low, and compute 21-day ATR%. `SYMBOL STOP` rows use your manual stop and fetch price/ATR%. The fallback stop is `today_low + min(max($0.10, price * 0.002), $1.00)`, rounded to cents. The default Alpaca feed is `iex`; use `--feed delayed_sip` or `--feed sip` only if your Alpaca plan supports the desired feed.
